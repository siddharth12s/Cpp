...
int f(int i, int target, vector<int> &v,vector<vector<int>> &dp){
    ...
    int notpick = f(i - 1, target, v,dp);
    int pick = INT_MAX;
    if(v[i]<=target){
        pick = 1 + f(i, target - v[i], v,dp);
    }
