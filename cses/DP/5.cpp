...
int f(int i, int target, vector<int> &digits, vector<vector<int>> &dp)
{

    if (i == 0)
    {

        if (target % digits[0] == 0)
        {
            return target/digits[0];
        }
        return 1e9;
    }
    ...
    int notpick = f(i - 1, target, digits, dp);
    int pick = 1e9;
    if (digits[i] <= target)
    {
        pick = f(i - 1, target - digits[i], digits, dp);
    }

    return dp[i][target] = min(pick, notpick);
}

int find_max_digit(int t){
    int maxi = INT_MIN;
    while (t > 0)
    {
        int rem = t % 10;
