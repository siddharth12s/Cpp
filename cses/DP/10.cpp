...
int f(std::string n, std::string m, int i, int j, std::vector<std::vector<int>> &dp)
{

    if (n[i] == m[j])
    {
        return f(n, m, i - 1, j - 1, dp);
    }
    ...
    int add = f(n, m, i, j - 1, dp);
    int remove = f(n, m, i - 1, j, dp);
    int replace = f(n, m, i - 1, j - 1, dp);

    return dp[i][j] = std::ranges::min(add, std::ranges::min(remove, replace)) + 1;
}
int main()
{
    ...
    // int ans = f(n, m, n.size() - 1, m.size() - 1, dp);
    int ans;

    for (auto j = 1; j <= m.size();j++){
        dp[0][j] = j;
    }
    ...
    for (auto i = 1; i <= n.size();i++){
        for (auto j = 1; j <= m.size();j++){
            if(n[i-1]==m[j-1]){
                ...
                dp[i][j] = std::ranges::min(dp[i][j - 1], std::ranges::min(dp[i - 1][j], dp[i - 1][j - 1])) + 1;
            }
        }
    }

    ans = dp[n.size()][m.size()];
    std::cout << ans;
