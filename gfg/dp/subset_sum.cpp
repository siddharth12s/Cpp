...
int f (std::vector<int> &v,int i, int sum, std::vector<std::vector<int>> &dp){
    ...
    int nottake = f(v, i - 1, sum,dp);
    int take = 0;

    if(v[i]<=sum){
        take = f(v, i - 1, sum - v[i], dp);
    }
...
int main(){
    ...
    // {
    //     int ans = f(v, i, sum, dp);
    //     if(ans){
    //         break;
    //     }
    // }
    ans = f(v, v.size()-1, sum, dp);
